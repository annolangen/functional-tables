...
export function makeDataFrame(
    size: number, header: string[], data: (string|number)[][]): DataFrame {
  ...
  const column_metadata =
      reader.map((r, i) => ({
                   name: header[i],
                   type: r === toNumber ? DataFrameColumnType.NUMERIC :
                                          DataFrameColumnType.STRING
                 } as DataFrameColumn));
  const columns = reader.map(
      (r, i) => r === toNumber ? Float64Array.from(data, row => r(row[i])) :
  ...
  return {
    ...
    columns,
    orderBy,
    slice: (offset: number, limit: number) => Array.from(
        Array(Math.min(size - offset, limit)),
        (_, row) => columns.map(c => c[row + offset]))
  };
  ...
  function orderBy(i: number, o: Order) {
    ...
    if (col instanceof Float64Array) {
      permutation.sort(
          o === Order.ASC ? (a: number, b: number) => col[a] - col[b] :
    ...
    return {
      slice: (offset: number, limit: number) => Array.from(
          Array(Math.min(size - offset, limit)),
          (_, row) => columns.map(col => col[permutation[row + offset]]))
    }
  }
}
