...
test('makeDiagonal', () => {
  ...
  expect(apply(m, Float64Array.of(7, 8, 9))).toEqual(
    Float64Array.of(7, 16, 27)
  );
});

test('makeLinearMapFromFloatGrid', () => {
  const m = makeLinearMapFromFloatGrid([
    Float64Array.of(0, 0),
    Float64Array.of(0, 1),
    Float64Array.of(1, 0),
  ]);
  ...
  expect(apply(m, Float64Array.of(8, 9))).toEqual(Float64Array.of(0, 9, 8));
});
